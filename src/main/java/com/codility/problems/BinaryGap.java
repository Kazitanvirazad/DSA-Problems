...
public class BinaryGap {
	public static void main(String[] args) {
		...
		long binaryGap = binaryGap(binaryNumber);
		System.out.println(binaryGap);
	}

	public static long binaryGap(long n) {
		long mainValue = n; // 1000010001

		long mainCounter = 0;
		long counter = 0;
		boolean isLastValueOne = false;
		boolean isEligibleForCount = false;

		while (mainValue != 0) {

			long currentValue = (mainValue % 10);
			mainValue /= 10;

			if (currentValue == 1) {
				isLastValueOne = true;
				isEligibleForCount = true;
				if (mainCounter < counter) {
					mainCounter = counter;
				}
			}
			if (currentValue == 0 && isEligibleForCount) {
				counter++;
				isLastValueOne = false;
			}
	...
	public static long convertToBinary(int n) {
		...
		long i = 1;
		while (mainValue != 0) {
			int quotent = mainValue / 2;
			int remaindar = mainValue % 2;
			if (remaindar != 0)
				result += remaindar * i;
			i *= 10;
			mainValue = quotent;
